...
export default React.createClass({
  ...
  fetchToken() {
    return fetch('/api/token').then((res) => {
      if (res.status !== 200) {
        throw new Error('Failed to receive auth token');
      }
      return res.text();
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        <h2 className="base--h2">Speech to Text Demo</h2>

        <div style={{ paddingRight: '3em', paddingBottom: '2em', }}
        >
          The following example uses the IBM Watson speech detection service to transcribe text to page. The result includes the recognized text, {' '}
          ...
          Some models can <a className="base--a" href="https://console.bluemix.net/docs/services/speech-to-text/output.html#output">detect multiple speakers</a>.
        </div>
        ...
        <div className="buttons">
          ...
          <button className={micButtonClass} onClick={this.handleMicClick}>
            <Icon type={this.state.audioSource === 'mic' ? 'stop' : 'microphone'} fill={micIconFill} /> Microphone
          </button>
